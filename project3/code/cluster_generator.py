...
if __name__ == "__main__":
    ...
    exit()
    p_list = [0.2, 0.5, 0.55, 0.58, 0.6, 0.62]
    plt.figure(num=0, figsize = (8,5), facecolor='w', edgecolor='k')
    for i in range(len(p_list)):
        plt.subplot(2,3,i+1)
        plt.title(f"p = {p_list[i]}")
        cluster = TwoDim_cluster(L,p_list[i])
        areaImg = cluster.area[cluster.lw]
        plt.imshow(areaImg, origin = "upper")
